...
##'
##' .. content for \details{} ..
##' @title mainSPOLreporting
##' @param file CHAR nom du fichier des donn?es brut dans le rep donnees
...
mainSPOLreporting <- function(file="Extrait.txt",fileDataClean="data.csv",fileData3sessions = "data3session.csv",
                              ...
                              seuilAvorteDuree= 3,seuilAvorteEvenement=4,seuilExclusionDelai = 10,
                              selectedSessionPlot=TRUE,carte = TRUE,abondanceRelative=TRUE ,variationAbondance=TRUE,variationAbondanceEspece=TRUE,
                              productivite=TRUE,proportionJeunes=TRUE,conditionCorporelle=TRUE,retour=TRUE,
 ...
 #file="Extrait.txt";fileDataClean="data.csv";fileData3sessions = "data3session.csv" #####
  ...
  #  seuilAbondanceAnneeAll=30;seuilAbondanceAnneeSite=10 #####
  #  seuilAvorteDuree= 4;seuilAvorteEvenement=5;seuilExclusionDelai = 10;
...
##
    ...
    if(importationData == "brut") {
        ...
        d <- import(file,firstYear=2007,lastYear,decimalData=".",fileDataClean,fileLog,
                    seuilAvorteDuree,seuilAvorteEvenement,seuilExclusionDelai)
        
        file <- paste("data_DB/",fileDataClean,sep="")
        require(readr)
        d <- read_delim(file, "\t", 
                        escape_double = FALSE, trim_ws = TRUE)
        #d <- read.table(file,sep="\t",dec=".",stringsAsFactor=FALSE,header=TRUE,encoding="UTF-8")
        catlog(c("  <-- ",file,"\n"),fileLog)
    ...
    d$NEW.ID_PROG <- gsub(" ", "-", d$NEW.ID_PROG) 
    d$NEW.ID_PROG <- gsub("_", "-", d$NEW.ID_PROG) 
    ...
    if(all) {
        ...
        if(abondanceRelative){
            ...
            catlog(c("\nABONDANCE RELATIVE GROUPE\n"),fileLog)
            groupsRelativeAbund.all(d,fileLog=fileLog,print.fig=FALSE,save.fig=TRUE,save.data_france=TRUE)
        }
        ...
        if(variationAbondanceEspece) {
            ...
            catlog(c("\nVARIATION ABONDANCE PAR CATEGORIE\n"),fileLog)
            abundanceCatYear.all(d,fileLog=fileLog,print.fig=FALSE,save.fig=TRUE,save.data_france=TRUE)
         }
        ...
        if(proportionJeunes) {
            ...
            proportionJuv.all(d,fileLog=fileLog,print.fig=FALSE,save.fig=TRUE,save.data_france=TRUE)
        ...
        if(retour) {
            ...
            print("Pas suffisamment de données régionales")
            catlog(c("\nTAUX DE RETOUR MENSUEL\n"),fileLog)
            MensualReturnRate.all(d,do.all=TRUE,do.sp=TRUE,seuilAbondanceAnnee=seuilAbondanceAnneeAll,fileLog=fileLog,print.fig=FALSE,save.fig=TRUE,save.data_france=TRUE)
            MensualReturnRate_2etapes.all(d,fileLog=fileLog,print.fig.janv=FALSE,print.fig.fev=FALSE,save.fig=TRUE,save.data_france=TRUE)
            catlog(c("\nDIFFERENCE MENSUELLE\n"),fileLog)
            MensualDiff.all(d,do.all=TRUE,fileLog="log.txt",print.fig.janv=FALSE,print.fig.fev=FALSE,save.fig=TRUE,save.data_france=TRUE)
...
## productivityYearSpecies.site(d,site=site,species="PHYCOL",nom_sp = "Pouillot v?loce",fileLog=fileLog,print.fig=TRUE,save.fig=FALSE,add_title=FALSE,facet=TRUE)
...
##
              ...
              d_body <- bodyCondition.site(d,site=site,community_level=FALSE,species_level=TRUE,species=NULL,nom_sp=NULL,fileLog=fileLog,print.fig=TRUE,save.fig=FALSE,add_title=FALSE,facet=TRUE,return.table=TRUE)
                ...
                plot_local_sp(d_body,print.fig=TRUE,save.fig=FALSE,facet_sp=TRUE,facet_group="AGE_first",y_lab="Condition corporelle: Masse/(Ecart ? la taille moyenne + 1)",x_lab="Ann?e",title_txt="",vecCol=c("#07307b","#0c5ef6","#c10909","#ea5d18"), minYear = min(subset(d,NEW.ID_PROG == site)$YEAR)-1,maxYear = max(subset(d,NEW.ID_PROG == site)$YEAR))
            }
            if(retour) {
                catlog(c("\nTAUX DE RETOUR\n"),fileLog)
